...
When the top most glass is full, any excess liquid poured will
fall equally to the glass immediately to the left and right of it.
When those glasses become full, any excess champagne will
...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        row = self._get_row(poured, query_row)

        if query_glass < 0 or query_glass >= len(row):
            return 0

        return 1 if row[query_glass] > 1 else row[query_glass]

    def _get_row(self, poured, query_row):
        if query_row == 0:
            return [poured]

        prior_row = self._get_row(poured, query_row - 1)
        current_row = [0 for _ in range(len(prior_row) + 1)]

        for i in range(len(prior_row)):
            if prior_row[i] > 1:
                excess = prior_row[i] - 1
                current_row[i] += excess / 2
                current_row[i + 1] += excess / 2

        return current_row


print(Solution().champagneTower(7, 3, 1))
