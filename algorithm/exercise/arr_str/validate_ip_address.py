...
class Solution:
    ...
    def valid_ip_address(self, IP):
        if IP.count(".") == 3 and all(self.is_ip_v4(seg) for seg in IP.split(".")):
            return "IPv4"

        if IP.count(":") == 7 and all(self.is_ip_v6(seg) for seg in IP.split(":")):
            return "IPv6"
...
print(Solution().valid_ip_address('2001:0db8:85a3:0:0:8A2E:0370:7334'))
